//
//  UIViewController+IRViewPopping.m
//  IRTransitionDemo
//
...
CG_INLINE CGRect    BkRectInRectWithAlignementOption(CGRect myRect, CGRect refRect,IRPopinAlignementOption option)
{
    switch (option) {
        case IRPopinAlignementOptionCentered:
            return BkRectCenterInRect(myRect,refRect);
            break;
            
        case IRPopinAlignementOptionUp:
            return BkRectAlignUpInRect(myRect,refRect);
            break;
            
        case IRPopinAlignementOptionLeft:
            return BkRectAlignLeftInRect(myRect,refRect);
            break;
            
        case IRPopinAlignementOptionDown:
            return BkRectAlignDownInRect(myRect,refRect);
            break;
            
        case IRPopinAlignementOptionRight:
...
@implementation IRBlurParameters
- (id)init
{
    self = [super init];
    if (self) {
...
@implementation UIViewController (IRViewPopping)
                    ...
                    completion:(void(^)(void))completion
{
    ...
    if (self.presentedPopinViewController == nil) {
        ...
        IRPopinOption options = [popinController popinOptions];
        if (! (options & IRPopinDisableAutoDismiss)) {
        ...
        if (options & IRPopinDimmingViewStyleNone) {
            [dimmingView setBackgroundColor:[UIColor clearColor]];
        } else if (options & IRPopinBlurryDimmingView && [self.view respondsToSelector:@selector(drawViewHierarchyInRect:afterScreenUpdates:)]) {
            UIImage *bgImage = [self createImageFromView:self.view];
            IRBlurParameters *parameters = [popinController blurParameters];
        ...
        if (! (options & IRPopinIgnoreKeyboardNotification)) {
...
{
    IRPopinTransitionDirection direction = popinController.popinTransitionDirection;
    CGRect frame = popinController.view.frame;
    if (direction == IRPopinTransitionDirectionTop || direction == IRPopinTransitionDirectionBottom) {
        CGFloat multiplier = (direction == IRPopinTransitionDirectionBottom) ? 1.0 : -1.0;
        CGFloat popinPosition = 0.0f;
        
        if (direction == IRPopinTransitionDirectionTop) {
            popinPosition = CGRectGetMaxY(frame);
        } else if (direction == IRPopinTransitionDirectionBottom) {
        ...
        CGFloat multiplier = (direction == IRPopinTransitionDirectionRight) ? 1.0 : -1.0;
        CGFloat popinPosition = 0.0f;
        
        if (direction == IRPopinTransitionDirectionLeft) {
            popinPosition = CGRectGetMaxX(frame);
        } else if (direction == IRPopinTransitionDirectionRight) {
...
- (void)addPopinToHierarchy:(UIViewController *)popinController
{
    ...
    //Add motion effect
    IRPopinOption options = [popinController popinOptions];
    if (NO == (options & IRPopinDisableParallaxEffect)) {
...
{
    IRBlurParameters *param = objc_getAssociatedObject(self, _cmd);
    if (nil == param) {
        return [[IRBlurParameters alloc] init];
    }
...
{
    IRPopinTransitionStyle transitionStyle = popinController.popinTransitionStyle;
    IRPopinTransitionDirection direction = popinController.popinTransitionDirection;
    switch (transitionStyle) {
        case IRPopinTransitionStyleCrossDissolve:
            return [self alphaInAnimationForPopinController:popinController toPosition:finalFrame];
        case IRPopinTransitionStyleSpringySlide:
        case IRPopinTransitionStyleSlide:
            return [self slideInAnimationForPopinController:popinController toPosition:finalFrame withDirection:direction];
        case IRPopinTransitionStyleSpringyZoom:
        case IRPopinTransitionStyleZoom:
            return [self zoomInAnimationForPopinController:popinController toPosition:finalFrame];
        case IRPopinTransitionStyleCustom:
...
//Slides
- (void (^)(void))slideInAnimationForPopinController:(UIViewController *)popinController toPosition:(CGRect)finalFrame withDirection:(IRPopinTransitionDirection)direction
{
    //Animation setup
...
- (void (^)(void))slideOutAnimationForPopinController:(UIViewController *)popinController withDirection:(IRPopinTransitionDirection)direction
{
    //Change properties values
    void (^animation)(void) = ^{
        popinController.view.frame = [self animationFrameForPopinController:popinController margin:0.0f];
...
//Snap
- (void (^)(void))snapInAnimationForPopinController:(UIViewController *)popinController toPosition:(CGRect)finalFrame withDirection:(IRPopinTransitionDirection)direction completion:(void(^)(void))completion
{
    //Animation setup
...
- (void (^)(void))snapOutAnimationForPopinController:(UIViewController *)popinController withDirection:(IRPopinTransitionDirection)direction completion:(void(^)(void))completion
{
...
- (void (^)(void))customInAnimationForPopinController:(UIViewController *)popinController toPosition:(CGRect)finalFrame withDirection:(IRPopinTransitionDirection)direction
{
...
- (void (^)(void))customOutAnimationForPopinController:(UIViewController *)popinController withDirection:(IRPopinTransitionDirection)direction
{
...
{
    return self.popinTransitionStyle == IRPopinTransitionStyleSnap && [self popinCanUseDynamics];
}
...
+ (CGFloat)dampingValueForTransitionStyle:(IRPopinTransitionStyle)transitionStyle
{
    CGFloat damping;
    switch (transitionStyle) {
        case IRPopinTransitionStyleSpringySlide:
...
+ (NSTimeInterval)animationDurationForTransitionStyle:(IRPopinTransitionStyle)transitionStyle
{
    NSTimeInterval animationDuration;
    switch (transitionStyle) {
        case IRPopinTransitionStyleSpringySlide:
